...
if not hasattr(st, 'model0'):
	st.model0 = BARTSummarization.load_from_checkpoint(CHECKPOINT_PATH_0,model_name=MODEL_NAME, tokenizer=st.tokenizer)
	st.model0.to(DEVICE)

if not hasattr(st, 'model1'):
    st.model1 = BARTSummarization.load_from_checkpoint(CHECKPOINT_PATH_1,model_name=MODEL_NAME, tokenizer=st.tokenizer)
    st.model1.to(DEVICE)

if not hasattr(st, 'model2'):
    st.model2 = BARTSummarization.load_from_checkpoint(CHECKPOINT_PATH_2,model_name=MODEL_NAME, tokenizer=st.tokenizer)
    st.model2.to(DEVICE)

if not hasattr(st, 'model3'):
    st.model3 = BARTSummarization.load_from_checkpoint(CHECKPOINT_PATH_3,model_name=MODEL_NAME, tokenizer=st.tokenizer)
    st.model3.to(DEVICE)
...
def encode_text_ent(input_text):
    ...
    for key, value in st.entity_dict():
    ...
    x_input_ids = torch.cat((torch.tensor([st.model2.tokenizer.convert_tokens_to_ids(st.entity)]), x['input_ids'].view(-1)), dim=0)
    x_attention_mask = torch.cat((torch.tensor([1]), x['attention_mask'].view(-1)), dim=0)
    return {"input_ids": x_input_ids.view(1, -1), "attention_mask": x_attention_mask.view(1, -1)}
...
def encode_text_both(input_text, length_bin):
    ...
    x_input_ids = torch.cat((torch.tensor([st.model3.tokenizer.convert_tokens_to_ids(length_bin), st.model3.tokenizer.convert_tokens_to_ids(st.entity)]), x['input_ids'].view(-1)), dim=0)
